...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadPost = async () => {
      try {
        ...
        let postData, commentsData: SetStateAction<Comment[]>;
        ...
        try {
          commentsData = await getPostComments(id);
        } catch (err) {
  ...
  const isPostAuthor = post && token ? post.author.id === post.authorId : false;
  
  // Check if the current user is the author of a comment
  const isCommentAuthor = (comment: Comment, currentUserId?: string) => {
        ...
        {post.imageUrl && !imgError && (
          <div className="relative aspect-video w-full overflow-hidden rounded-lg mb-8">
            <Image
              ...
              unoptimized={true} // This can help with external images
                ...
                <Card key={comment.id}>
                  <CardContent className="pt-6">
                    <div className="flex items-start justify-between">
                      ...
                      {token && isCommentAuthor(comment, comment.authorId) && (
                        <Button
                          variant="ghost"
                          size="sm"
